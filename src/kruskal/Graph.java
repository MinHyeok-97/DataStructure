...
public class Graph {
    ...
    public void undirectedEdge(int u, int v, int w) {
        // TODO : 간선을 추가할 때 부터 가중치 순으로 정렬하기 위해
        // 우선순위 큐를 사용한다.
    ...
    private int find(int x) {
        ...
        if (disjointSet[x] == x) {
             ...
             *  3 -> 2 -> 1 -> 0 순으로 재귀적인 탐색을 해야한다.
             *
             *  첫 번째 탐색 시 대표 원소를 바로 찾지 못하나 대표 원소로 변경해준다면
    ...
    public Set<Edge> getMST() {
        ...
        while (!priorityQueue.isEmpty()) {
            ...
            // TODO : 대표 원소가 서로 다르면 서로 다른 부분 집합이다.
            if (find(edge.u) != find(edge.v)) {
                /**
                 * 두 원소가 같은 부분 집합에 속해 있지 않을 때만
                 * union 연산 후, 간선을 안전 간선으로 추가한다
                 */
                union(edge.u, edge.v);
